...
import { Download, Upload, FileCode, Smartphone, Globe, AlertCircle } from 'lucide-react';
...
export const ExportPanel = ({ code, user }: ExportPanelProps) => {
  ...
  const checkDailyUploads = async () => {
    ...
    try {
      const { data, error } = await supabase.rpc('check_daily_upload_limit', {
        user_id_param: user.id
      });

      if (error) throw error;
      setDailyUploads(data || 0);
    } catch (error) {
  ...
  const canUpload = userRole === 'admin' || dailyUploads < 2;
  ...
  const handleDirectUpload = async () => {
    if (!user) {
      toast.error('Please login to upload websites');
      return;
    }
    ...
    if (!canUpload) {
      toast.error('You have reached your daily upload limit (2 websites per day)');
      return;
    }
    ...
    try {
      ...
      const { error: dbError } = await supabase
        ...
        .insert({
          ...
          description: 'Generated by Zeus AI Coder',
          file_name: fileName,
          file_url: publicUrl,
      ...
      await supabase
        ...
        .insert({
          user_id: user.id,
          action: 'website_uploaded',
          details: { file_name: fileName, file_url: publicUrl },
        });
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              {!user && (
                <p className="text-sm text-yellow-400 mt-1">Login required</p>
              )}
              {user && !canUpload && (
          ...
          <button
            ...
            disabled={isExporting || !code || !user || !canUpload}
